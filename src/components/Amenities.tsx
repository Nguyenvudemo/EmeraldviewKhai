        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
          {highlights.map((item, index) => (
            <motion.div
              ...
              className="bento-card bento-amber-border bg-emerald-900/40 p-6 sm:p-8 flex flex-col space-y-4 group relative"
            >
              {/* Gold gradient line accent on hover */}
              <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-gold-400 to-transparent opacity-0 group-hover:opacity-100 transition-opacity"></div>
              
              {/* Icon */}
              <div className="w-14 h-14 bg-emerald-950 rounded-xl flex items-center justify-center border border-emerald-800 shadow-md group-hover:scale-110 transition-transform duration-300">
                {item.icon}
              </div>

              {/* Title & Description */}
              <div className="space-y-2">
                <h3 className="font-serif font-bold text-lg text-gold-100 group-hover:text-gold-200 transition-colors">
                  {item.title}
                </h3>
